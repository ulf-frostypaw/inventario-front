...
export const Button = ({ children, variant, onClick}) => {
    return (
        <button onClick={onClick} className={"w-full py-3 px-4 sm:py-3.5 sm:px-6 text-sm sm:text-base font-medium relative h-auto inline-flex items-center justify-center rounded-full transition-colors outline-none " + styles[variant]}>{children}</button>
    );
};
